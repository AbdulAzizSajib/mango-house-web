...
const inp =
  "w-full px-3 py-2.5 rounded-xl border border-border bg-input text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all";

const BN_DIGITS = ["০", "১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯"];
const toBn = (n: number) =>
  String(n).replace(/\d/g, (d) => BN_DIGITS[Number(d)]);

const toBnDate = (iso: string) => {
...
const EMPTY_FORM = { name: "", comment: "", location: "", date: todayISO() };
...
export default function Testimonials() {
  ...
  const set = (k: keyof FormState, v: string) => {
    ...
    setForm((f) => ({ ...f, [k]: clean.slice(0, limits[k]) }));
  };

  const nameValid = (v: string) => /^[ঀ-৿a-zA-Z\s.'-]+$/.test(v.trim());
  const locationValid = (v: string) => /^[ঀ-৿a-zA-Z\s,.-]+$/.test(v.trim());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      return;
    if (!nameValid(form.name)) {
      setStatus("error");
      setErrMsg("নামে শুধু বাংলা বা ইংরেজি অক্ষর ব্যবহার করুন");
      return;
    }
    if (!locationValid(form.location)) {
      setStatus("error");
      setErrMsg("এলাকায় শুধু বাংলা বা ইংরেজি অক্ষর ব্যবহার করুন");
      return;
    }
    setStatus("loading");
    setErrMsg("");
  ...
  return (
    <section
      id="reviews"
      className="relative py-20 sm:py-28 px-4 sm:px-6 border-t border-border/40 overflow-hidden"
    >
              ...
              {featured.map((r, i) => (
                ...
                >
                  <blockquote className="font-display text-2xl sm:text-3xl lg:text-[34px] leading-[1.45] text-foreground/85 text-center max-w-3xl mx-auto">
                    <span className="text-primary/70 mr-1" aria-hidden>
                      "
                    </span>
                    {r.comment}
                    <span className="text-primary/70 ml-1" aria-hidden>
                      "
                    </span>
                  </blockquote>
              ...
              {marqueeItems.map((m, i) => (
                ...
                >
                  <p className="font-display text-[15px] leading-[1.55] text-foreground/85 mb-3">
                    <span className="text-primary/70 mr-0.5" aria-hidden>
                      "
                    </span>
                    {m.comment}
                    <span className="text-primary/70 ml-0.5" aria-hidden>
                      "
                    </span>
                  </p>
                  <p className="font-mono text-[10px] tracking-[0.14em] uppercase text-foreground/55">
                    —{" "}
                    <span className="font-body tracking-normal normal-case text-[12px] text-foreground/70 mr-2">
                      {m.name}
                    </span>
                    <span className="font-body tracking-normal normal-case text-[12px] text-foreground/70">
                      ({toBnDate(m.date)})
                    </span>
                    <span className="mx-1.5 text-foreground/30">·</span>
                    <span className="font-body tracking-normal normal-case text-[12px] text-foreground/55">
                      {m.location}
                    </span>
                  </p>
                </article>
      ...
      <div ref={formRef} className="mt-20 sm:mt-28 max-w-xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h2 className="font-display text-2xl sm:text-3xl font-medium text-foreground mt-2">
            রিভিউ লিখুন
          </h2>
          <p className="text-sm text-foreground/55 mt-1.5">
            আমাদের আম কেমন লেগেছে জানালে আমরা খুশি হবো
          </p>
        </div>
        ...
        {status === "success" ? (
          <div className="bg-secondary/10 border border-secondary/30 rounded-2xl px-6 py-10 text-center">
            ...
            <p className="text-sm text-foreground/60 mt-1.5 mb-5">
              আমরা রিভিউটি যাচাই করে প্রকাশ করবো।
            </p>
            <button
          ...
          <form
            ...
            className="bg-card border border-border/60 rounded-2xl p-6 sm:p-8 space-y-4"
          >
            {status === "error" && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="font-mono text-[10px] tracking-[0.16em] uppercase text-foreground/55 mb-1.5 block">
                  তারিখ <span className="text-primary">*</span>
                </label>
                <input
                  disabled
                  type="date"
              ...
              <div>
                <label className="font-mono text-[10px] tracking-[0.16em] uppercase text-foreground/55 mb-1.5 block">
                  এলাকা <span className="text-primary">*</span>
                </label>
                <input
                  value={form.location}
